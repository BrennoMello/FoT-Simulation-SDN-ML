...
path="/home/mininet/projeto_ml/FoT-Stream_Simulation"

def return_hosts():
	f=open(path+'/reg/data_hosts.json','r')
...
def write_host(st):
	x=open(path+'/reg/data_hosts.json','a')
	x.write(st+"\n")
	x.close()
...
def return_association_server():
	lines = open(path+'/association_gateway_server.json','r')
	devices=[]
	for l in lines:
		#print(l)
...
def return_association():
	f=open(path+'/reg/association_hosts.json','r')
	lines=len(f.readlines())
	f.close()
	f=open(path+'/reg/association_hosts.json','r')
	st2=[]
